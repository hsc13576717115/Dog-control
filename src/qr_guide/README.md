...
这一节非常重要。后续如果给 `FSM` 增加 `q + qd + tau` 联合控制，必须始终遵守这里的方向约定，避免位置命令、速度命令和力矩命令互相打架。

统一约定如下：

- `LowlevelCmd.motorCmd[i].q / dq / tau` 都解释为**用户关节空间**命令
- `IOSDK` 对 `q / dq / tau` **不额外翻符号**
- `IOSDK` 只负责：
  - `q`：加校准偏置、乘减速比
  - `dq`：乘减速比
  - `tau`：直接透传
- 左右腿 / 前后腿的符号差异来自 `LegKinematics` 内部的镜像规则，而不是 `IOSDK`

当前镜像规则：

- 前腿：`hip(q0)` 镜像
- 左腿：`thigh(q1)`、`calf(q2)` 镜像
...
一句话说：**`q / dq / tau` 在软件语义上必须同向一致。**

下面这张表给出在当前站立工作点附近，关节给一个正小量时，足端大致往哪边走。坐标系仍然是：
...
1. `LegKinematics` 的镜像规则是否仍然成立
2. `jacobian(q)`、`jointVelocityFromFootVelocity(...)`、`jointTorquesFromFootForce(...)` 是否仍然使用同一用户关节方向
3. `IOSDK` 是否只做减速比和零位偏置，而没有偷偷再翻一次 `dq / tau`

如果以后出现“位置方向正确，但速度或力矩方向相反”的问题，优先回看这一节，而不是先改控制器增益。
