...
当前整狗模型现在采用“白色圆角机身盒体 + 参数化机械腿部”的方式：机身按你自己的 `body_size_m` 直接显示并做了圆角处理，髋/大腿/小腿/足端都按你自己的 `hip_mount / L0 / L1 / L2 / foot_radius` 直接生成，因此可视化长度和连接关系与真实参数一致；外观上只参考 Go2 的层次感，不再一比一复用它的腿部模型。当前腿部视觉件统一为白色圆柱连杆、白色脚垫球体和黑色扁平关节。
